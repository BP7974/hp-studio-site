...
async function sendVerificationEmail({ to, code, ttlMinutes = DEFAULT_VERIFICATION_TTL }) {
  const transport = getTransport();
  const config = getSmtpConfig();
  ...
  const subject = 'HP Studio 注册验证码';
  const text = `您的验证码是 ${code}，${ttlMinutes} 分钟内有效，请勿泄露给他人。`;
  const html = `
