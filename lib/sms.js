...
const DEFAULT_SMS_TEMPLATE = '【HP Studio】验证码 {code}，{ttl} 分钟内有效，请勿泄露。';
...
function resolveSpugEndpoint() {
  ...
  if (SPUG_SMS_BASE_URL && (SPUG_SMS_PATH || SPUG_SMS_TEMPLATE_ID)) {
    try {
      const relativePath = SPUG_SMS_PATH || `/send/${SPUG_SMS_TEMPLATE_ID}`;
      const url = new URL(relativePath, SPUG_SMS_BASE_URL);
      return url.toString();
    } catch (err) {
...
async function sendSpugSms({ phone, code, ttlMinutes }) {
  ...
  const payload = {
    name: SPUG_SMS_NAME || 'HP Studio',
    code,
    targets: phone
  };

  if (SPUG_SMS_EXTRA_JSON) {
    try {
      Object.assign(payload, JSON.parse(SPUG_SMS_EXTRA_JSON));
    } catch (err) {
      console.warn('[SMS] 解析 SPUG_SMS_EXTRA_JSON 失败：', err.message);
    }
  }
...
async function sendSmsVerification({ phone, code, ttlMinutes, messageTemplate }) {
  const provider = getProvider();
  const formattedMessage = formatTemplate(messageTemplate || DEFAULT_SMS_TEMPLATE, {
    code,
    ttl: ttlMinutes
  });
  if (provider === 'twilio') {
